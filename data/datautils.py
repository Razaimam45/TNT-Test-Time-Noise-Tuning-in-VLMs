...
from data.cifar10c import CIFAR10C
...
def build_dataset(set_id, transform, data_root, mode='test', n_shot=None, split="all", bongard_anno=False,
                    corruption="defocus_blur", level="1"):
    if set_id == 'I':
    ...
    elif set_id == 'cifar10c':
        testset = CIFAR10C(
                    os.path.join(data_root, 'cifar10-c'),
                    corruption, transform=transform
                )
    else:
...
def augmix_Raza(image, preaugment, preprocess, aug_list, severity=1, normalize=None):
    # preaugment = get_preaugment()
    # x_tensor = image # tensor [0,1]
    # noised_tensor = torch.clamp(x_tensor ) # adding noise to tensor image
    x_processed = preaugment(image) # augmented
    # x_processed = normalize(x_processed)
    if len(aug_list) == 0:
        return x_processed
    
def augmix_asif(image, preaugment, preprocess, aug_list, severity=1):
    # preaugment = get_preaugment()
    x_orig = preaugment(image)
    x_processed = preprocess(x_orig)
    if len(aug_list) == 0:
...
def augmix_source_noise(image, preaugment, aug_list, severity=1):
    # preaugment = get_preaugment()
    x_aug = preaugment(image)
    # x_processed = preprocess(x_orig)
    if len(aug_list) == 0:
        return x_aug
...
class AugMixAugmenter_Raza(object):
    def __init__(self, base_transform, preprocess, n_views=2, augmix=False, 
                 severity=1, hard_aug=False, noise=None):
        self.base_transform = base_transform
        self.preprocess = preprocess
        self.n_views = n_views
        self.aug_list = augmentations.augmentations if augmix else []
    ...
    def __call__(self, x): # x: PIL image
        # noise = torch.rand((3, 224, 224))
        # self.set_noise(self.noise)
        
        image = self.preprocess(self.base_transform(x)) # x in range of [0,1]
        # if self.noise is not None:
        #     image = self.normalize(image + self.noise)
        # else:
        #     image = self.normalize(image)
        # noise = torch.zeros_like(image)
        # views = [augmix_Raza(self.base_transform(x), self.preaugment, self.preprocess, self.aug_list, self.severity, normalize=self.normalize, noise=self.noise) for _ in range(self.n_views)]
        # return [image] + views
        return image
    

class AugMixAugmenter_Asif(object):
    def __init__(self, base_transform, preprocess, n_views=2, augmix=False, 
                    severity=1, hard_aug=False):
        self.base_transform = base_transform
        self.preprocess = preprocess
    ...
    def __call__(self, x):
        image = self.preprocess(self.base_transform(x))
        views = [augmix_asif(x, self.preaugment, self.preprocess, self.aug_list, self.severity) for _ in range(self.n_views)]
        return [image] + views
    
class AugMixAugmenter_source_image(object):
    def __init__(self, base_transform, preprocess, n_views=2, augmix=False, 
                    severity=1, hard_aug=False):
        self.base_transform = base_transform
        self.preprocess = preprocess
